...
public class ColGen {
	...
	private static Network getNetwork(int[] hubs, int l) {
		...
		for (int i = 0; i < nVar; i++) {
			for (int j = i + 1; j < nVar; j++) {
				double RTUB = getUpperbound(i , j, hubs, l); // the upper bound obtained by the greedy algorithm
	...
	public static double getUpperbound( int i, int j, int[] hList, int l ) {
		...
		// destination.
		PriorityQueue<Route> feasibleRoutes = RoutingTree.getFeasibleRoutes2(i, j, hList, nodes, routes,	distances, alpha);
		...
		while (!feasibleRoutes.isEmpty() && cntr <= lastIndex) {
			...
			PriorityQueue<Route> feasibleRoutes1 = new PriorityQueue<Route>(feasibleRoutes); 
			PriorityQueue<Route> feasibleRoutes2 = new PriorityQueue<Route>(feasibleRoutes); 

			if (output.routes[cntr] != null && !output.routes[cntr].i.equals(output.routes[cntr].k)
					&& !isFinal(output.routes[cntr])) {
				// List of feasible routes to select right child node from.
				ArrayList<Node> usedHubs1 = new ArrayList<Node>(output.usedHubs.get(cntr)); 
				...
				while (output.routes[leftNodeIndex] == null) {
					...
					if (!usedHubs1.contains(selectedRoute.k) && !usedHubs1.contains(selectedRoute.m)) {
			...
			if (output.routes[cntr] != null && !output.routes[cntr].j.equals(output.routes[cntr].m)
					&& !isFinal(output.routes[cntr]) && !output.routes[cntr].k.equals(output.routes[cntr].m)) {
				ArrayList<Node> usedHubs2 = new ArrayList<Node>(output.usedHubs.get(cntr)); // make
																							// a
																							// list
																							// of
																							// parent
				...
				while (output.routes[rightNodeIndex] == null) {
					...
					if (!usedHubs2.contains(selectedRoute.k) && !usedHubs2.contains(selectedRoute.m)) {
	...
	/**
	 * computes a lower bound on the expected cost of the routing tree
	 * going from i to j give hub combination hList with up to l 
	...
	public static double getLowerbound( int i, int j, int[] hList, int l ) {
		...
		while ( cntr <= lastIndex ) {
			...
			if (output.routes[cntr] != null && !output.routes[cntr].j.equals(output.routes[cntr].m)
				...
				ArrayList<Node> usedHubs2 = new ArrayList<Node>(output.usedHubs.get(cntr)); 
	 ...
	 * @return RoutingTree
	 */
	public static RoutingTree getRoutingTree(int i, int j, int[] hList, int l, double UB) {
		...
		// generating list of feasible routes between the origin and the
		// destination.
		ArrayList<Route> feasibleRoutes = RoutingTree.getFeasibleRoutes(i, j, hList, nodes, routes, distances, alpha);
